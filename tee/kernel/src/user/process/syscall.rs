...
use self::{
    args::{
        ...
        Offset, OpenFlags, Pipe2Flags, Pointer, Pollfd, ProtFlags, RtSigprocmaskHow,
...
fn poll(
    ...
    vm_activator.activate(&virtual_memory, |vm| {
        for i in 0..nfds {
            ...
            vm.read_bytes(fds.get() + i * 8, bytes_of_mut(&mut pollfd))?;
        }
        Result::<_>::Ok(())
    })?;
...
fn execve(
    ...
    let (pathname, args, envs) = vm_activator.activate(&virtual_memory, |vm| -> Result<_> {
        ...
        if args.iter().any(|a| a.to_string_lossy() == "+x") {
            args.push(CString::new("-v").unwrap());
        }
...
fn exit(
    ...
    debug!("tid={} exit({status})", thread.tid());
...
fn unlink(
    ...
    let pathname = vm_activator.activate(&virtual_memory, |vm| vm.read(pathname))?;
    unlink_file(thread.cwd.clone(), &pathname)?;
    Ok(0)
}
...
fn symlink(
    ...
    create_link(thread.cwd.clone(), &newname, oldname)?;
...
async fn exit_group(thread: Arc<Thread>, status: u64) -> SyscallResult {
    ...
    debug!("pid={} exit_group({status})", process.pid);
