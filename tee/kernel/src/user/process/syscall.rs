...
fn execve(
    ...
    let (pathname, args, mut envs) = vm_activator.activate(&virtual_memory, |vm| -> Result<_> {
...
fn faccessat(
    ...
    let path = thread.cwd.path(&mut ctx);

    debug!(
        "{:?}",
        path.as_ref().map(|a| core::str::from_utf8(a.as_bytes()))
    );
