...
use crate::kvm::{KvmGuestMemFdFlags, Page, SevHandle, VmHandle};
...
impl Slot {
    pub fn for_launch_update(
        ...
        let restricted_fd = if sev_handle.is_some() {
            Some(
    ...
    pub fn new(vm: &VmHandle, gpa: PhysFrame, sev_handle: Option<&SevHandle>) -> Result<Self> {
        let len = 512 * 0x1000;
